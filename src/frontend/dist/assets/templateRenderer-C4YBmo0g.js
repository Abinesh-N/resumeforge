...
function expItems(experience, titleColor = "#1a1a1a", dateColor = "#666", companyColor = "#444") {
...
function eduItems(education, titleColor = "#1a1a1a", dateColor = "#666") {
...
function projItems(projects, titleColor = "#1a1a1a", linkColor = "#2563eb") {
  return projects.map(
    (p, i) => `
    <div style="margin-bottom:10px;">
...
function skillPills(skills, bg = "#f3f4f6", color = "#374151", border = "") {
  return skills.map(
    (s, i) => `<span data-editable="skills.${i}" style="background:${bg};color:${color};${border ? `border:1px solid ${border};` : ""}padding:2px 9px;border-radius:3px;font-size:8.5pt;display:inline-block;margin:2px 3px 2px 0;cursor:pointer;">${esc(s)}</span>`
  ).join("");
}
function skillDots(skills, dotColor = "#2563eb") {
  return skills.map(
    (s, i) => `<div style="font-size:9pt;color:#333;margin-bottom:4px;"><span style="color:${dotColor};margin-right:6px;">●</span><span data-editable="skills.${i}" style="cursor:pointer;">${esc(s)}</span></div>`
  ).join("");
}
function html(head, body, extra = "") {
...
function tModernClean(c) {
  ...
  const sec = (t) => `<div style="font-size:9.5pt;font-weight:800;color:#2563eb;letter-spacing:0.08em;text-transform:uppercase;border-left:3px solid #2563eb;padding-left:8px;margin-bottom:10px;">${t}</div>`;
  const wrap = (t, inner) => inner ? `<div style="margin-bottom:20px;">${sec(t)}${inner}</div>` : "";
  const b = [
...
function tModernDark(c) {
  ...
  const sec = (t) => `<div style="font-size:9.5pt;font-weight:700;color:#00D4FF;letter-spacing:0.1em;text-transform:uppercase;margin-bottom:10px;"><span style="color:#00D4FF;">✦</span> ${t}</div>`;
  const hr = `<div style="border-bottom:1px solid #e5e7eb;margin-bottom:12px;"></div>`;
  const wrap = (t, inner) => inner ? `<div style="margin-bottom:20px;">${sec(t)}${hr}${inner}</div>` : "";
  const b = [
...
function tModernNavy(c) {
  ...
  const sec = (t) => `<div style="font-size:9.5pt;font-weight:700;color:#1E3A5F;letter-spacing:0.08em;text-transform:uppercase;border-bottom:2px solid #1E3A5F;padding-bottom:4px;margin-bottom:10px;">${t}</div>`;
  const wrap = (t, inner) => inner ? `<div style="margin-bottom:22px;">${sec(t)}${inner}</div>` : "";
  const b = [
...
function tModernTeal(c) {
  ...
  const sec = (t) => `<div style="font-size:9pt;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:#0D9488;border-bottom:2px solid #0D9488;padding-bottom:3px;margin-bottom:10px;">${t}</div>`;
  const wrap = (t, inner) => inner ? `<div style="margin-bottom:20px;">${sec(t)}${inner}</div>` : "";
  const b = [
...
function tModernSlate(c) {
  ...
  const sec = (t) => `<div style="font-size:9pt;font-weight:700;color:#334155;letter-spacing:0.08em;text-transform:uppercase;border-bottom:2px solid #334155;padding-bottom:3px;margin-bottom:10px;">${t}</div>`;
  const wrap = (t, inner) => inner ? `<div style="margin-bottom:20px;">${sec(t)}${inner}</div>` : "";
...
function tModernGradient(c) {
  ...
  const sec = (t) => `<div style="font-size:9.5pt;font-weight:700;color:#7C3AED;letter-spacing:0.08em;text-transform:uppercase;margin-bottom:10px;padding-left:8px;border-left:3px solid #7C3AED;">${t}</div>`;
  const wrap = (t, inner) => inner ? `<div style="margin-bottom:20px;">${sec(t)}${inner}</div>` : "";
  const b = [
...
function tModernBold(c) {
  ...
  const sec = (t) => `<div style="margin-bottom:12px;"><div style="font-size:10pt;font-weight:900;color:#000;letter-spacing:0.04em;text-transform:uppercase;">${t}</div><div style="height:3px;background:#F59E0B;margin-top:3px;"></div></div>`;
  const wrap = (t, inner) => inner ? `<div style="margin-bottom:22px;">${sec(t)}${inner}</div>` : "";
  const b = [
    info.summary ? `<div style="margin-bottom:20px;font-size:9pt;color:#374151;line-height:1.6;">${ed("personalInfo.summary", info.summary)}</div>` : "",
...
function tModernCompact(c) {
  ...
  const sec = (t) => `<span style="background:#e2e8f0;color:#475569;font-size:7.5pt;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;padding:1px 7px;border-radius:10px;">${t}</span>`;
  const wrap = (title, inner) => inner ? `<div style="margin-bottom:13px;">${sec(title)}<div style="margin-top:7px;">${inner}</div></div>` : "";
  const exp = c.experience.map(
    (e, i) => `<div style="margin-bottom:9px;"><div style="display:flex;justify-content:space-between;"><span style="font-weight:700;font-size:9.5pt;">${ed(`experience.${i}.title`, e.title)}</span><span style="font-size:8pt;color:#64748b;">${ed(`experience.${i}.startDate`, e.startDate)}–${ed(`experience.${i}.endDate`, e.endDate)}</span></div><div style="font-size:8.5pt;color:#475569;">${ed(`experience.${i}.company`, e.company)}</div>${e.description ? `<div style="font-size:8.5pt;color:#4b5563;line-height:1.5;">${ed(`experience.${i}.description`, e.description)}</div>` : ""}</div>`
  ).join("");
  const edu = c.education.map(
    (e, i) => `<div style="margin-bottom:8px;"><div style="display:flex;justify-content:space-between;"><span style="font-weight:700;font-size:9.5pt;">${ed(`education.${i}.degree`, e.degree)}</span><span style="font-size:8pt;color:#64748b;">${ed(`education.${i}.startDate`, e.startDate)}–${ed(`education.${i}.endDate`, e.endDate)}</span></div><div style="font-size:8.5pt;color:#475569;">${ed(`education.${i}.school`, e.school)}</div></div>`
  ).join("");
  const b = [
    info.summary ? `<div style="margin-bottom:12px;font-size:9pt;color:#374151;">${ed("personalInfo.summary", info.summary)}</div>` : "",
    wrap("Experience", exp),
    wrap("Education", edu),
    wrap("Projects", projItems(c.projects, "#1e293b", "#2563eb")),
    wrap(
      "Skills",
      c.skills.map(
        (s, i) => `<span data-editable="skills.${i}" style="font-size:8.5pt;color:#374151;margin-right:12px;cursor:pointer;">${esc(s)}</span>`
      ).join("")
    )
  ].join("");
  return html("", page(hdr + b, "28px 32px"));
}
function tProClassic(c) {
  const info = c.personalInfo;
  ...
  const sec = (t) => `<div style="font-size:9.5pt;font-weight:700;color:#374151;letter-spacing:0.12em;text-transform:uppercase;text-align:center;border-bottom:1px solid #d1d5db;padding-bottom:3px;margin-bottom:10px;">${t}</div>`;
  const wrap = (t, inner) => inner ? `<div style="margin-bottom:20px;">${sec(t)}${inner}</div>` : "";
  const b = [
    info.summary ? `<div style="text-align:center;font-size:9pt;color:#4b5563;font-style:italic;margin-bottom:18px;line-height:1.6;">${ed("personalInfo.summary", info.summary)}</div>` : "",
...
function tProExecutive(c) {
  ...
  const sec = (t) => `<div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;"><div style="font-size:9.5pt;font-weight:800;color:#111827;letter-spacing:0.06em;text-transform:uppercase;">${t}</div><div style="flex:1;height:1px;background:#D97706;"></div></div>`;
  const wrap = (t, inner) => inner ? `<div style="margin-bottom:22px;">${sec(t)}${inner}</div>` : "";
  const b = [
...
function tProCorporate(c) {
  ...
  const sec = (t) => `<div style="background:#003366;color:#fff;font-size:8.5pt;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;padding:4px 10px;margin-bottom:10px;display:inline-block;">${t}</div>`;
  const wrap = (t, inner) => inner ? `<div style="margin-bottom:20px;">${sec(t)}<div style="border-left:3px solid #003366;padding-left:12px;">${inner}</div></div>` : "";
  const b = [
    info.summary ? `<div style="margin-bottom:18px;font-size:9pt;color:#374151;line-height:1.6;">${ed("personalInfo.summary", info.summary)}</div>` : "",
    wrap(
...
function tProFormal(c) {
  ...
  const sec = (t) => `<div style="font-size:9pt;font-weight:700;color:#111827;letter-spacing:0.1em;text-transform:uppercase;font-variant:small-caps;margin-bottom:6px;border-bottom:1px solid #9ca3af;padding-bottom:2px;">${t}</div>`;
  const wrap = (t, inner) => inner ? `<div style="margin-bottom:18px;">${sec(t)}${inner}</div>` : "";
  const b = [
    info.summary ? `<div style="font-size:9pt;color:#4b5563;line-height:1.65;font-style:italic;margin-bottom:18px;">${ed("personalInfo.summary", info.summary)}</div>` : "",
...
function tProElegant(c) {
  ...
  const sec = (t) => `<div style="font-size:9.5pt;font-weight:700;color:#9B2335;letter-spacing:0.06em;text-transform:uppercase;padding-left:10px;border-left:3px solid #9B2335;margin-bottom:10px;">${t}</div>`;
  const wrap = (t, inner) => inner ? `<div style="margin-bottom:20px;">${sec(t)}${inner}</div>` : "";
  const b = [
...
function tProRefined(c) {
  ...
  const sec = (t) => `<div style="font-size:9pt;font-weight:700;color:#166534;letter-spacing:0.08em;text-transform:uppercase;border-bottom:2px solid #166534;padding-bottom:3px;margin-bottom:10px;">${t}</div>`;
  const wrap = (t, inner) => inner ? `<div style="margin-bottom:20px;">${sec(t)}${inner}</div>` : "";
...
function tProPolished(c) {
  const info = c.personalInfo;
  const stripes = Array(8).fill(0).map(
    (_, i) => `<div style="position:absolute;top:0;left:${i * 12}%;width:12%;height:100%;background:rgba(255,255,255,${i % 2 === 0 ? "0.06" : "0"});"></div>`
  ).join("");
  ...
  const sec = (t) => `<div style="display:flex;align-items:center;gap:8px;margin-bottom:10px;"><div style="font-size:9pt;font-weight:700;color:#1e3a5f;letter-spacing:0.08em;text-transform:uppercase;">${t}</div><div style="flex:1;height:1px;background:#94a3b8;"></div></div>`;
  const wrap = (t, inner) => inner ? `<div style="margin-bottom:20px;">${sec(t)}${inner}</div>` : "";
  const b = [
    info.summary ? `<div style="margin-bottom:18px;font-size:9pt;color:#374151;line-height:1.6;">${ed("personalInfo.summary", info.summary)}</div>` : "",
...
function tProDistinguished(c) {
  ...
  const sec = (t) => `<div style="border-left:4px solid #EA580C;padding-left:10px;font-size:9.5pt;font-weight:800;color:#111827;letter-spacing:0.06em;text-transform:uppercase;margin-bottom:10px;">${t}</div>`;
  const wrap = (t, inner) => inner ? `<div style="margin-bottom:22px;">${sec(t)}${inner}</div>` : "";
  const b = [
...
function tCreativeColor(c) {
  ...
  const sec = (t) => `<div style="font-size:9.5pt;font-weight:700;color:#4F46E5;letter-spacing:0.08em;text-transform:uppercase;border-bottom:2px solid #4F46E5;padding-bottom:3px;margin-bottom:10px;">${t}</div>`;
  const wrap = (t, inner) => inner ? `<div style="margin-bottom:20px;">${sec(t)}${inner}</div>` : "";
...
function tCreativeVibrant(c) {
  ...
  const sec = (t) => `<div style="font-size:9.5pt;font-weight:700;color:#F97316;letter-spacing:0.08em;text-transform:uppercase;margin-bottom:10px;"><span style="display:inline-block;width:20px;height:3px;background:#F97316;vertical-align:middle;margin-right:8px;"></span>${t}</div>`;
  const wrap = (t, inner) => inner ? `<div style="margin-bottom:20px;">${sec(t)}<div style="border-left:2px solid #fed7aa;padding-left:12px;">${inner}</div></div>` : "";
  const b = [
...
function tCreativeBold(c) {
  ...
  const sec = (t) => `<div style="font-size:10pt;font-weight:900;color:#22C55E;letter-spacing:0.04em;text-transform:uppercase;margin-bottom:10px;padding-bottom:4px;border-bottom:2px solid #22C55E;">${t}</div>`;
  const wrap = (t, inner) => inner ? `<div style="margin-bottom:22px;">${sec(t)}${inner}</div>` : "";
  const b = [
...
function tCreativeDesigner(c) {
  ...
  const skillTags = c.skills.length ? `<div style="margin-bottom:20px;"><div style="font-size:9pt;font-weight:700;color:#EC4899;letter-spacing:0.08em;text-transform:uppercase;margin-bottom:8px;">Skills & Tools</div><div>${skillPills(c.skills, "#fdf2f8", "#9d174d", "#f9a8d4")}</div></div>` : "";
  const sec = (t) => `<div style="font-size:9.5pt;font-weight:700;color:#EC4899;letter-spacing:0.08em;text-transform:uppercase;margin-bottom:10px;">${t}</div>`;
...
function tCreativeArtistic(c) {
  ...
  const sec = (t, color) => `<div style="border-left:8px solid ${color};padding-left:14px;margin-bottom:12px;"><div style="font-size:9.5pt;font-weight:800;color:${color};letter-spacing:0.08em;text-transform:uppercase;">${t}</div></div>`;
  const wrap = (t, inner, color) => inner ? `<div style="margin-bottom:22px;">${sec(t, color)}<div style="padding-left:22px;">${inner}</div></div>` : "";
  const b = [
    wrap(
      "Experience",
...
function tMinimalPure(c) {
  ...
  const b = [
    ...
    wrap(
      "Projects",
      c.projects.map(
        (p, i) => `<div style="margin-bottom:9px;"><div style="font-size:10pt;color:#111827;">${ed(`projects.${i}.title`, p.title)}</div>${p.description ? `<div style="font-size:9pt;color:#6b7280;margin-top:2px;">${ed(`projects.${i}.description`, p.description)}</div>` : ""}</div>`
      ).join("")
    )
  ].join("");
  return html(
...
function tMinimalClean(c) {
  ...
  const sec = (t) => `<div style="font-size:8.5pt;font-weight:600;color:#6b7280;letter-spacing:0.15em;text-transform:uppercase;font-variant:small-caps;margin-bottom:8px;">${t}</div>`;
  const wrap = (t, inner) => inner ? `<div style="margin-bottom:20px;">${sec(t)}${inner}</div>` : "";
  const exp = c.experience.map(
    (e, i) => `<div style="margin-bottom:12px;"><div style="display:flex;justify-content:space-between;"><span style="font-weight:700;font-size:10pt;">${ed(`experience.${i}.title`, e.title)} <span style="color:#059669;">@ ${ed(`experience.${i}.company`, e.company)}</span></span><span style="font-size:8.5pt;color:#059669;">${ed(`experience.${i}.startDate`, e.startDate)}–${ed(`experience.${i}.endDate`, e.endDate)}</span></div>${e.description ? `<div style="font-size:9pt;color:#374151;line-height:1.55;margin-top:3px;">${ed(`experience.${i}.description`, e.description)}</div>` : ""}</div>`
  ).join("");
  const edu = c.education.map(
...
function tMinimalZen(c) {
  ...
  const sec = (t) => `<div style="text-align:center;font-size:9pt;font-weight:600;color:#92400E;letter-spacing:0.18em;text-transform:uppercase;margin:20px 0 12px;"><span style="padding:0 16px;position:relative;">${t}</span></div>`;
  const wrap = (t, inner) => inner ? `<div>${sec(t)}${inner}</div>` : "";
  const b = [
    info.summary ? `<div style="text-align:center;font-size:9pt;color:#6b7280;line-height:1.7;max-width:580px;margin:0 auto 24px;">${ed("personalInfo.summary", info.summary)}</div>` : "",
...
function tMinimalSwiss(c) {
  ...
  const wrap = (t, inner) => inner ? `${sec(t)}${inner}</div></div>` : "";
  const b = [
    info.summary ? `<div style="margin-bottom:22px;display:grid;grid-template-columns:60px 1fr;gap:16px;"><div style="font-size:11pt;font-weight:900;color:#DC2626;">00</div><div style="font-size:9pt;color:#4b5563;line-height:1.65;">${ed("personalInfo.summary", info.summary)}</div></div>` : "",
...
function tMinimalNordic(c) {
  ...
  const sec = (t) => `<div style="font-size:8.5pt;font-weight:600;color:#64748B;letter-spacing:0.12em;text-transform:uppercase;border-bottom:1px solid #e7e5e4;padding-bottom:4px;margin-bottom:10px;">${t}</div>`;
  const wrap = (t, inner) => inner ? `<div style="margin-bottom:20px;">${sec(t)}${inner}</div>` : "";
  const exp = c.experience.map(
    (e, i) => `<div style="margin-bottom:12px;"><div style="display:flex;justify-content:space-between;"><span style="font-weight:600;font-size:10pt;color:#1c1917;">${ed(`experience.${i}.title`, e.title)}</span><span style="font-size:8.5pt;color:#78716c;">${ed(`experience.${i}.startDate`, e.startDate)}–${ed(`experience.${i}.endDate`, e.endDate)}</span></div><div style="font-size:9pt;color:#57534e;">${ed(`experience.${i}.company`, e.company)}</div>${e.description ? `<div style="font-size:9pt;color:#57534e;line-height:1.55;margin-top:3px;">${ed(`experience.${i}.description`, e.description)}</div>` : ""}</div>`
  ).join("");
  const b = [
    wrap("Experience", exp),
...
function tMinimalSimple(c) {
  ...
  const sec = (t) => `<div style="font-size:9.5pt;font-weight:700;color:#000;text-transform:uppercase;letter-spacing:0.04em;margin-bottom:6px;margin-top:4px;">${t}</div><div style="border-bottom:1px solid #000;margin-bottom:8px;"></div>`;
  const wrap = (t, inner) => inner ? `<div style="margin-bottom:14px;">${sec(t)}${inner}</div>` : "";
  const exp = c.experience.map(
    (e, i) => `<div style="margin-bottom:9px;"><div style="display:flex;justify-content:space-between;"><span style="font-weight:700;font-size:9.5pt;">${ed(`experience.${i}.title`, e.title)}, ${ed(`experience.${i}.company`, e.company)}</span><span style="font-size:9pt;">${ed(`experience.${i}.startDate`, e.startDate)}–${ed(`experience.${i}.endDate`, e.endDate)}</span></div>${e.description ? `<div style="font-size:9pt;color:#333;margin-top:2px;">${ed(`experience.${i}.description`, e.description)}</div>` : ""}</div>`
  ).join("");
  const edu = c.education.map(
    (e, i) => `<div style="margin-bottom:8px;"><div style="display:flex;justify-content:space-between;"><span style="font-weight:700;font-size:9.5pt;">${ed(`education.${i}.degree`, e.degree)}, ${ed(`education.${i}.school`, e.school)}</span><span style="font-size:9pt;">${ed(`education.${i}.startDate`, e.startDate)}–${ed(`education.${i}.endDate`, e.endDate)}</span></div></div>`
  ).join("");
  const b = [
    info.summary ? `<div style="font-size:9pt;color:#333;line-height:1.6;margin-bottom:14px;">${ed("personalInfo.summary", info.summary)}</div>` : "",
    wrap("Experience", exp),
    wrap("Education", edu),
    wrap(
      "Skills",
...
function tTwocolModern(c) {
  ...
  const sec = (t) => `<div style="font-size:9pt;font-weight:700;color:#3B82F6;letter-spacing:0.08em;text-transform:uppercase;border-bottom:2px solid #3B82F6;padding-bottom:3px;margin-bottom:10px;">${t}</div>`;
  const wrap = (t, inner) => inner ? `<div style="margin-bottom:20px;">${sec(t)}${inner}</div>` : "";
  const right = `<div style="flex:1;padding:28px 24px;">${[
...
function tTwocolClassic(c) {
  ...
  const sec = (t) => `<div style="font-size:9pt;font-weight:700;color:#1E3A5F;letter-spacing:0.08em;text-transform:uppercase;border-bottom:2px solid #1E3A5F;padding-bottom:3px;margin-bottom:10px;">${t}</div>`;
  const wrap = (t, inner) => inner ? `<div style="margin-bottom:18px;">${sec(t)}${inner}</div>` : "";
...
function tTwocolSkills(c) {
  const info = c.personalInfo;
  const skillBars = c.skills.map((s, i) => {
  ...
  const sec = (t) => `<div style="font-size:9pt;font-weight:700;color:#14532D;letter-spacing:0.08em;text-transform:uppercase;border-bottom:2px solid #14532D;padding-bottom:3px;margin-bottom:10px;">${t}</div>`;
  const wrap = (t, inner) => inner ? `<div style="margin-bottom:18px;">${sec(t)}${inner}</div>` : "";
  const right = `<div style="flex:1;padding:28px 22px;">${hdr}${[
...
function tTwocolSidebar(c) {
  ...
  const sec = (t) => `<div style="font-size:9pt;font-weight:700;color:#F59E0B;letter-spacing:0.08em;text-transform:uppercase;border-bottom:2px solid #F59E0B;padding-bottom:3px;margin-bottom:10px;">${t}</div>`;
  const wrap = (t, inner) => inner ? `<div style="margin-bottom:18px;">${sec(t)}${inner}</div>` : "";
...
function tTwocolAccent(c) {
  ...
  const sec = (t) => `<div style="font-size:9pt;font-weight:700;color:#F97316;letter-spacing:0.08em;text-transform:uppercase;margin-bottom:10px;">${t}</div>`;
  const wrap = (t, inner) => inner ? `<div style="margin-bottom:18px;">${sec(t)}${inner}</div>` : "";
...
function tAcademicTraditional(c) {
  const info = c.personalInfo;
  ...
  const sec = (t) => `<div style="font-size:9.5pt;font-weight:700;color:#111827;letter-spacing:0.12em;text-transform:uppercase;border-bottom:1.5px solid #374151;padding-bottom:2px;margin-bottom:10px;margin-top:18px;">${t}</div>`;
  const wrap = (t, inner) => inner ? `<div>${sec(t)}<div style="padding-left:12px;">${inner}</div></div>` : "";
  const exp = c.experience.map(
    (e, i) => `<div style="margin-bottom:12px;"><div style="display:flex;justify-content:space-between;"><div><span style="font-weight:700;font-style:italic;font-size:9.5pt;">${ed(`experience.${i}.title`, e.title)}</span>, <span style="font-size:9.5pt;">${ed(`experience.${i}.company`, e.company)}</span></div><span style="font-size:9pt;">${ed(`experience.${i}.startDate`, e.startDate)}–${ed(`experience.${i}.endDate`, e.endDate)}</span></div>${e.description ? `<div style="font-size:9pt;color:#374151;margin-top:3px;">${ed(`experience.${i}.description`, e.description)}</div>` : ""}</div>`
  ).join("");
  const b = [
    info.summary ? `<div style="font-size:9pt;color:#374151;font-style:italic;line-height:1.65;margin-bottom:14px;">${ed("personalInfo.summary", info.summary)}</div>` : "",
...
function tAcademicResearch(c) {
  ...
  const sec = (t) => `<div style="font-size:9.5pt;font-weight:700;color:#0D9488;letter-spacing:0.08em;text-transform:uppercase;margin-bottom:10px;padding-bottom:3px;border-bottom:1.5px solid #0D9488;">${t}</div>`;
  const wrap = (t, inner) => inner ? `<div style="margin-bottom:20px;">${sec(t)}${inner}</div>` : "";
  const b = [
    wrap("Education", eduItems(c.education, "#111827", "#0D9488")),
    wrap(
...
function tAcademicCv(c) {
  ...
  const sec = (t) => `<div style="font-size:9.5pt;font-weight:700;color:#1E3A8A;letter-spacing:0.08em;text-transform:uppercase;border-bottom:1px solid #1E3A8A;padding-bottom:3px;margin-bottom:10px;">${t}</div>`;
  const wrap = (t, inner) => inner ? `<div style="margin-bottom:20px;">${sec(t)}${inner}</div>` : "";
  const b = [
    info.summary ? `<div style="font-size:9pt;color:#374151;line-height:1.65;margin-bottom:18px;font-style:italic;">${ed("personalInfo.summary", info.summary)}</div>` : "",
...
function tExecPremium(c) {
  ...
  const sec = (t) => `<div style="display:flex;align-items:center;gap:12px;margin-bottom:12px;"><div style="font-size:9.5pt;font-weight:800;color:#78350f;letter-spacing:0.06em;text-transform:uppercase;">${t}</div><div style="flex:1;height:1.5px;background:linear-gradient(to right,#D97706,transparent);"></div></div>`;
  const wrap = (t, inner) => inner ? `<div style="margin-bottom:22px;">${sec(t)}${inner}</div>` : "";
  const b = [
...
function tExecLeadership(c) {
  ...
  const sec = (t) => `<div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;"><div style="width:4px;height:16px;background:#059669;"></div><div style="font-size:9.5pt;font-weight:700;color:#374151;letter-spacing:0.06em;text-transform:uppercase;">${t}</div></div>`;
  const wrap = (t, inner) => inner ? `<div style="margin-bottom:20px;">${sec(t)}${inner}</div>` : "";
  const b = [
    info.summary ? `<div style="font-size:9.5pt;color:#374151;line-height:1.65;margin-bottom:20px;padding:12px 16px;background:#f9fafb;border-left:4px solid #059669;">${ed("personalInfo.summary", info.summary)}</div>` : "",
...
function tStartupModern(c) {
  ...
  const sec = (t) => `<div style="font-size:8.5pt;color:#16A34A;font-family:monospace;margin-bottom:8px;"><span style="color:#6b7280;">// </span><span style="font-weight:700;letter-spacing:0.05em;">${t.toUpperCase()}</span></div><div style="height:1px;background:#e5e7eb;margin-bottom:10px;"></div>`;
  const wrap = (t, inner) => inner ? `<div style="margin-bottom:20px;">${sec(t)}${inner}</div>` : "";
  const skillLine = c.skills.length ? `<div style="margin-bottom:20px;"><div style="font-size:8.5pt;color:#16A34A;font-family:monospace;margin-bottom:8px;"><span style="color:#6b7280;">// </span><span style="font-weight:700;">SKILLS</span></div><div style="height:1px;background:#e5e7eb;margin-bottom:10px;"></div>${skillPills(c.skills, "#eff6ff", "#1d4ed8", "#bfdbfe")}</div>` : "";
  const b = [
...
function tStartupTech(c) {
  ...
  const sec = (t) => `<div style="color:#22C55E;font-size:9pt;font-weight:700;font-family:monospace;margin-bottom:8px;padding:2px 0;border-bottom:1px solid #374151;"># ${t.toLowerCase()}</div>`;
  const wrap = (t, inner) => inner ? `<div style="margin-bottom:20px;">${sec(t)}${inner}</div>` : "";
  const exp = c.experience.map(
    (e, i) => `<div style="margin-bottom:12px;"><div style="display:flex;justify-content:space-between;"><span style="font-weight:700;font-size:9.5pt;color:#f3f4f6;">${ed(`experience.${i}.title`, e.title)}</span><span style="font-size:8.5pt;color:#22C55E;font-family:monospace;">${ed(`experience.${i}.startDate`, e.startDate)}–${ed(`experience.${i}.endDate`, e.endDate)}</span></div><div style="font-size:9pt;color:#9ca3af;">${ed(`experience.${i}.company`, e.company)}</div>${e.description ? `<div style="font-size:9pt;color:#d1d5db;line-height:1.55;margin-top:3px;">${ed(`experience.${i}.description`, e.description)}</div>` : ""}</div>`
  ).join("");
  const edu = c.education.map(
    (e, i) => `<div style="margin-bottom:9px;"><div style="display:flex;justify-content:space-between;"><span style="font-weight:700;font-size:9.5pt;color:#f3f4f6;">${ed(`education.${i}.degree`, e.degree)}</span><span style="font-size:8.5pt;color:#22C55E;font-family:monospace;">${ed(`education.${i}.startDate`, e.startDate)}–${ed(`education.${i}.endDate`, e.endDate)}</span></div><div style="font-size:9pt;color:#9ca3af;">${ed(`education.${i}.school`, e.school)}</div></div>`
  ).join("");
  const projs = c.projects.map(
    (p, i) => `<div style="margin-bottom:9px;"><span style="font-weight:700;font-size:9.5pt;color:#22C55E;">${ed(`projects.${i}.title`, p.title)}</span>${p.link ? `<span style="font-size:8pt;color:#6b7280;margin-left:8px;font-family:monospace;">${ed(`projects.${i}.link`, p.link)}</span>` : ""}${p.description ? `<div style="font-size:9pt;color:#d1d5db;margin-top:2px;">${ed(`projects.${i}.description`, p.description)}</div>` : ""}</div>`
  ).join("");
  const skillsRow = c.skills.length ? `<div style="margin-bottom:20px;">${sec("Skills")}<div>${c.skills.map((s, i) => `<span data-editable="skills.${i}" style="background:#1f2937;color:#22C55E;font-family:monospace;font-size:8pt;padding:2px 8px;border:1px solid #374151;border-radius:2px;display:inline-block;margin:2px 3px 2px 0;cursor:pointer;">${esc(s)}</span>`).join("")}</div></div>` : "";
  const body = `<div style="padding:24px 50px;background:#111827;">${[
    info.summary ? `<div style="margin-bottom:18px;font-size:9pt;color:#9ca3af;line-height:1.6;font-family:monospace;padding:10px;background:#000;border-left:3px solid #22C55E;">${ed("personalInfo.summary", info.summary)}</div>` : "",
    wrap("Experience", exp),
    wrap("Projects", projs),
    wrap("Education", edu),
    skillsRow
